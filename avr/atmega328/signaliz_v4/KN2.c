...
volatile static uint8_t f_long=0;
		...
		if (BIC(PIN_KN2,KN2)) 	//CHANGE	//если кнопка нажата
			goto kn_a;						
		return;						
kn_a:
...
counting_kn:		
		...
		if (f_5==1) 				//если кнопка была уже нажата	
			goto kn_b;					
		if ((BIC(PIN_KN2,KN2)))	//CHANGE	// проверка случайного нажатия			
			goto kn_c;						
		else
			goto clean_flags;							
kn_c:	
...
kn_b:
		if (f_2==0)						// 0 - было нажатие, 1 - было отпускание
			...
			goto release_kn;
repeat_reading:					
		if ((BIC(PIN_KN2,KN2))) //CHANGE	// повторное считывание кнопок
			goto kn_d;						
		else
			goto kn_e;						
kn_d:	
		counter=2;							//!!! задержка №2 (повторная) при нажатой кнопке 
		cnt_long++;
