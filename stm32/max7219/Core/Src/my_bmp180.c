...
//------------
	case 20://pause
		StartGTimer(timer_pause_temp,10);
		if (ExpGTimer(timer_pause_temp)) {		//ждем 10 мс для получения темп-ры
		//vTaskDelay(5);
			i2c_alg=40;
...
//------------------
	case 60://pause
		//vTaskDelay(30);
		StartGTimer(timer_pause_temp,30);
		if (ExpGTimer(timer_pause_temp)) {		//ждем 10 мс для получения темп-ры
...
//SEI
	...
	TFTDrawUT32(48,132,GREEN,FONE,P_BMP,3,3);
	//TFTDrawChar(64,50,GREEN,FONE,'%',2);
	TFTDrawUT32(48,158,GREEN,FONE,T_BMP/10,2,3);
	TFTDrawChar(96,158,GREEN,FONE,'.',3);
	TFTDrawUT32(120,158,GREEN,FONE,T_BMP%10,1,3);
	TFTDrawChar(144,158,GREEN,FONE,'C',3);
